"""调用 STGS_Fastgs 文件夹内的方法代码，渲染多视角轨迹并编码为 MP4。"""
...
DEFAULT_METHOD_ROOT = Path("/home/swf/code/STGS_Fastgs/SpacetimeGaussians")
DEFAULT_OUTPUT = "stgs_fastgs_multiview.mp4"


def resolve_input_path(path: Path, method_root: Path) -> Path:
    """相对路径优先按调用目录解析，不存在时再按 STGS_Fastgs 根目录解析。"""
    if path.is_absolute():
        return path.resolve()
...
def run_local_multiview_test(test_args):
    """在 STGS_Fastgs 进程内替换多视角相机加载器，再运行方法目录的 test.py。"""
...
def main():
    ...
    print("STGS_Fastgs 渲染命令：", subprocess.list2cmdline(display_command))
    if args.dry_run:
        return

    if not args.skip_render:
        if not test_script.is_file():
            raise FileNotFoundError(f"STGS_Fastgs 方法入口不存在：{test_script}")
