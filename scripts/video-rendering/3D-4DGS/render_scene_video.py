"""渲染 3D-4DGS 场景，并编码为 GitHub 可播放的 MP4。"""
...
def encode_video(frame_dir: Path, output: Path, fps: int, pattern: str) -> None:
    ...
    if ffmpeg:
        with tempfile.TemporaryDirectory(prefix="3d4dgs_video_") as temp_dir:
...
def parse_args():
    ...
    parser.add_argument("--config", type=Path, required=True,
                        help="configs 下对应场景的 YAML 配置")
    parser.add_argument("--checkpoint", type=Path, required=True,
                        help="例如 chkpnt6000.pth")
...
def main():
    ...
    output = args.output or Path(
        "3d-4dgs_multiview.mp4" if args.multiview else "3d-4dgs.mp4"
    )
    ...
    if args.multiview:
        command.extend([
            "--render_multiview", "--multiview_frames", str(args.multiview_frames),
        ])
    ...
    for path, label in ((config_path, "配置文件"), (checkpoint_path, "检查点文件")):
        if not path.is_file():
            raise FileNotFoundError(f"{label}不存在：{path}")
