"""渲染 Ex4DGS 场景，并编码为 GitHub 可播放的 MP4。"""
...
def find_latest_frames(model_path: Path, split: str) -> Path:
    candidates = [path for path in (model_path / split).glob("itrs_*/renders")
                  if path.is_dir()]
    if not candidates:
        raise FileNotFoundError(f"未找到 {model_path / split / 'itrs_*' / 'renders'}")
...
def encode_video(frame_dir: Path, output: Path, fps: int, pattern: str) -> None:
    ...
    if ffmpeg:
        with tempfile.TemporaryDirectory(prefix="ex4dgs_video_") as temp_dir:
...
def main():
    ...
    output = args.output or Path(
        "ex4dgs_multiview.mp4" if args.multiview else "ex4dgs.mp4"
    )
